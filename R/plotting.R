cerego_matching_plot <- function(results.summary, 
								 error_bar_factor = 1.96,
								 plot_holdouts = FALSE,
								 x_limits, 
								 x_labels) {
	...
	# if(!missing(x_limits)) {
	# 	x_labels <- ifelse(missing(x_labels), x_limits, x_labels)
...
cerego_strata_plot <- function(results.summary, 
							   error_bar_factor = 1.96,
							   x_limits, 
							   x_labels) {
	results.summary$Sig <- cut(
	...
	p <- ggplot(results.summary, aes(x=Outcome, y=Estimate.Strata)) + 
